...
SELFMUTE_SETTINGS_PATH = (
    os.getenv("ALT_SELFMUTE_SETTINGS_PATH") or "config/selfmute_settings.yml"
)
...
class Selfmute(commands.Cog):
    ...
    async def perform_mute(
        self, member: discord.Member, mute_role: discord.Role, unmute_time: datetime
    ):
        no_remove_list = (
            selfmute_settings["selfmute_config"]
            .get(member.guild.id, {})
            .get("roles_not_to_remove", [])
        )
        roles_not_to_remove = (
            [member.guild.get_role(role_id) for role_id in no_remove_list]
            if no_remove_list
            else []
        )
        roles_to_save = [
        ...
        await self.bot.RUN(
            ...
            (
                ...
                member.id,
                mute_role.id,
                current_roles_string,
                unmute_time_string,
            ),
        )
        new_roles = [role for role in member.roles if role not in roles_to_save] + [
            mute_role
        ]
        await member.edit(roles=new_roles)

    @discord.app_commands.command(
        name="unmute_user", description="Removes a mute from a user."
    )
    ...
    async def unmute_user(
        ...
        mute_data = await self.bot.GET_ONE(
            GET_USER_MUTE_QUERY, (interaction.guild.id, member.id)
        )

        if not mute_data:
            await interaction.followup.send(
                "This user was not found in the muted data. Removing muted role.",
                ephemeral=True,
            )
        ...
        if mute_data:
            await interaction.followup.send(
                f"{member.mention} has been unmuted and roles restored when possible.",
                ephemeral=True,
            )

    async def perform_user_unmute(
        self, member: discord.Member, channel: discord.TextChannel, mute_data
    ):
        all_self_mute_role_ids = (
            selfmute_settings["selfmute_config"]
            .get(member.guild.id, {})
            .get("mute_roles", [])
        )
        all_selftmute_roles = [
            member.guild.get_role(role_id) for role_id in all_self_mute_role_ids
        ]
        await member.edit(
            roles=[role for role in member.roles if role not in all_selftmute_roles]
        )
        if not mute_data:
            return
        ...
        if role_ids_to_restore:
            roles_to_restore = [
                ...
                for role_id in role_ids_to_restore.split(",")
            ]
            ...
            if channel:
                await channel.send(
                    f"**🕒 Unmuted {member.mention} and restored the roles when possible.. 🕒**",
                    allowed_mentions=discord.AllowedMentions.none(),
                )
        await self.bot.RUN(REMOVE_MUTE_QUERY, (guild_id, user_id))

    @discord.app_commands.command(
        name="selfmute", description="Mute yourself for a specified amount of time."
    )
    @discord.app_commands.guild_only()
    async def selfmute(
        self,
        ...
        if hours < 0 or minutes < 0:
            await interaction.followup.send(
                "You can't mute yourself for a negative amount of time.", ephemeral=True
            )
            return
        if hours > 168:
            await interaction.followup.send(
                "You can only mute yourself for a maximum of 7 days.", ephemeral=True
            )
            return
        ...
        all_selftmute_roles = [
            interaction.guild.get_role(role_id) for role_id in all_self_mute_role_ids
        ]

        if not all_selftmute_roles:
            await interaction.followup.send(
                "This server has no selfmute roles configured.", ephemeral=True
            )
            return
        ...
        async def mute_callback(interaction: discord.Interaction):
            ...
            if announce_channel:
                await announce_channel.send(
                    mute_message, allowed_mentions=discord.AllowedMentions.none()
                )
        ...
        await interaction.followup.send(
            "Select a role to mute yourself with.", view=my_view, ephemeral=True
        )

    @discord.app_commands.command(
        name="check_mute",
        description="Removes your mute if the specified time has already pasted",
    )
    async def check_mute(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        if not interaction.guild:
            mute_data = await self.bot.GET(
                GET_ALL_USER_MUTES_QUERY, (interaction.user.id,)
            )
        else:
            mute_data = await self.bot.GET(
                GET_USER_MUTE_QUERY, (interaction.guild.id, interaction.user.id)
            )
        if not mute_data and interaction.guild:
            await self.perform_user_unmute(
                interaction.user, interaction.channel, mute_data
            )
            await interaction.followup.send("You are not muted.", ephemeral=True)
            return
        for mute_data_guild in mute_data:
            guild_id, user_id, mute_role_id, role_ids_to_restore, unmute_time = (
                mute_data_guild
            )
            mute_guild = self.bot.get_guild(guild_id)
            unmute_time = datetime.strptime(unmute_time, "%Y-%m-%d %H:%M:%S").replace(
                tzinfo=timezone.utc
            )
            if unmute_time > discord.utils.utcnow():
                await interaction.followup.send(
                    f"You are muted until <t:{int(unmute_time.timestamp())}:F>"
                    + f"which is <t:{int(unmute_time.timestamp())}:R> on `{mute_guild.name}`.",
                    ephemeral=True,
                )
            else:
                announce_channel_id = (
                ...
                await self.perform_user_unmute(
                    interaction.user, announce_channel, mute_data
                )
                await interaction.followup.send(
                    "You are not muted anymore.", ephemeral=True
                )
        ...
        for guild in self.bot.guilds:
            ...
            for mute_data in active_mutes:
                ...
                if unmute_time > discord.utils.utcnow().replace():
                    ...
                    if member:
                        await self.perform_user_unmute(
                            member, announce_channel, mute_data
                        )
                    else:
                        await self.bot.RUN(REMOVE_MUTE_QUERY, (guild_id, user_id))
