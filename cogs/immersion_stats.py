...
GET_USER_LOGS_FOR_PERIOD_WITH_NAME_QUERY_WITH_MEDIA_TYPE = GET_USER_LOGS_FOR_PERIOD_WITH_NAME_QUERY_BASE + " AND media_type = ? ORDER BY log_date;"
...
def set_plot_styles():
    ...
    emoji_font_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'fonts', 'NotoEmoji-VariableFont_wght.ttf')
...
def generate_wrapped_image(
    ...
    gs = fig.add_gridspec(3, 3, hspace=0.15, wspace=0.15, left=0.05, right=0.93, top=0.82, bottom=0.05)
    ...
    fig.text(0.54, title_y, wrapped_title, fontsize=42, fontweight='bold', color='#c6d0f5', 
            ...
            if response.status_code == 200:
                ...
                if avatar_img.mode in ('RGBA', 'LA', 'P'):
                    ...
                    if avatar_img.mode == 'P':
                        ...
                        avatar_img = avatar_img.convert('RGBA')
                    background.paste(avatar_img, mask=avatar_img.split()[-1] if avatar_img.mode == 'RGBA' else None)
                ...
                # Add circular border
                border = patches.Circle((0.5, 0.5), 0.48, transform=ax_avatar.transAxes,
                                       fill=False, edgecolor='#7287fd', linewidth=3)
    ...
    def create_stat_box(ax, title, value, subtitle="", color='#7287fd'):
        ...
        rect = patches.Rectangle((0, 0), 1, 1, linewidth=2, edgecolor=color, 
                                facecolor='#232634', transform=ax.transAxes)
        ax.add_patch(rect)
        
        # Title
        ax.text(0.5, 0.75, title, fontsize=18, fontweight='bold', color='#8b8c8c',
                ha='center', va='center', transform=ax.transAxes, fontproperties=font_prop)
        
        # Value
        ax.text(0.5, 0.45, value, fontsize=32, fontweight='bold', color=color,
                ha='center', va='center', transform=ax.transAxes, fontproperties=font_prop)
        
        # Subtitle
        if subtitle:
            ax.text(0.5, 0.15, subtitle, fontsize=14, color='#c6d0f5',
                    ha='center', va='center', transform=ax.transAxes, fontproperties=font_prop)
    ...
    create_stat_box(ax1, "⏱️ Total Time", time_str, f"{total_time:.1f} minutes", '#7287fd')
    ...
    if top_category is not None:
        ...
        category_time_str = f"{category_hours}h {category_minutes}m" if category_hours > 0 else f"{category_minutes}m"
        category_color = MEDIA_TYPES[category_name].get('color', '#7287fd')
        
        ax2 = fig.add_subplot(gs[0, 1])
        create_stat_box(ax2, "🏆 Top Category", category_name, 
    ...
    if most_logged_item is not None and most_logged_item.get('media_name'):
        ...
        item_time_str = f"{item_hours}h {item_minutes}m" if item_hours > 0 else f"{item_minutes}m"
        ...
        rect = patches.Rectangle((0, 0), 1, 1, linewidth=2, edgecolor='#ef9f76', 
                                facecolor='#232634', transform=ax3.transAxes)
        ax3.add_patch(rect)
        
        ax3.text(0.5, 0.75, "📚 Most Logged", fontsize=18, fontweight='bold', color='#8b8c8c',
                ha='center', va='center', transform=ax3.transAxes, fontproperties=font_prop)
        ...
        for i, line in enumerate(lines):
            ...
            ax3.text(0.5, y_pos, line, fontsize=20, fontweight='bold', color='#ef9f76',
                    ha='center', va='center', transform=ax3.transAxes, fontproperties=font_prop)
        
        ax3.text(0.5, 0.15, f"{item_entries} log{'s' if item_entries > 1 else ''} • {item_time_str}",
                fontsize=14, color='#c6d0f5', ha='center', va='center', 
                transform=ax3.transAxes, fontproperties=font_prop)
    
    avg_hours = int(avg_daily_time // 60)
    avg_minutes = int(avg_daily_time % 60)
    avg_time_str = f"{avg_hours}h {avg_minutes}m" if avg_hours > 0 else f"{avg_minutes}m"
    ax4 = fig.add_subplot(gs[1, 0])
    create_stat_box(ax4, "📈 Daily Average", avg_time_str,
                   f"{avg_daily_time:.1f} min/day", '#a6d189')
    
    ax5 = fig.add_subplot(gs[1, 1])
    create_stat_box(ax5, "📆 Consistency", f"{days_with_logs_percentage:.1f}%",
                   f"{logged_days} of {total_days_in_period} days", '#e78284')
    
    ax6 = fig.add_subplot(gs[1, 2])
    create_stat_box(ax6, "🔥 Longest Streak", f"{longest_streak} days",
                   "Consecutive days logged", '#f2d5cf')
    ...
    if len(category_stats) > 0:
        ...
        # Title
        ax7.text(0.5, 0.95, "📋 Category Breakdown", fontsize=24, fontweight='bold',
                color='#c6d0f5', ha='center', transform=ax7.transAxes, fontproperties=font_prop)
        ...
        for idx, (_, row) in enumerate(category_stats.head(6).iterrows()):
            ...
            percentage = (cat_time / total_time_for_pct * 100) if total_time_for_pct > 0 else 0
            ...
            ax7.text(x_pos, y_pos + 0.08, cat_name, fontsize=18, fontweight='bold',
                    color=cat_color, ha='left', transform=ax7.transAxes, fontproperties=font_prop)
            
            # Stats
            stats_text = f"{cat_time:.1f} min ({percentage:.1f}%) • {cat_amount:.0f} {unit_name}{'s' if cat_amount > 1 else ''}"
            ax7.text(x_pos, y_pos, stats_text, fontsize=14, color='#8b8c8c',
                    ha='left', transform=ax7.transAxes, fontproperties=font_prop)
        ...
        ax8.set_title("", fontsize=18, fontweight='bold', color='#c6d0f5', 
                     pad=10, fontproperties=font_prop)
    
    buffer = io.BytesIO()
    plt.savefig(buffer, format='png', facecolor=fig.get_facecolor(), bbox_inches='tight', dpi=150)
    buffer.seek(0)
    plt.close(fig)
...
class ImmersionLogMe(commands.Cog):
    ...
    async def get_user_logs_with_name(self, user_id, from_date, to_date, immersion_type=None):
        if immersion_type:
    ...
    @discord.app_commands.command(name='log_wrapped', description='Your year in immersion stats!')
    @discord.app_commands.describe(
    ...
    @discord.app_commands.choices(immersion_type=LOG_CHOICES)
    async def log_wrapped(self, interaction: discord.Interaction, user: Optional[discord.User] = None, from_date: Optional[str] = None, to_date: Optional[str] = None, immersion_type: Optional[str] = None):
        await interaction.response.defer()
        ...
        user_logs = await self.get_user_logs_with_name(user_id, from_date, to_date, immersion_type)
        logs_df = pd.DataFrame(user_logs, columns=['media_type', 'media_name', 'amount_logged', 'time_logged', 'log_date'])
        logs_df['log_date'] = pd.to_datetime(logs_df['log_date'])
        logs_df = logs_df.set_index('log_date')
        ...
        category_stats = period_df.groupby('media_type').agg({
            'time_logged': 'sum',
            'amount_logged': 'sum'
        }).reset_index()
        ...
        if not period_df['media_name'].isna().all():
            item_stats = period_df[period_df['media_name'].notna()].groupby('media_name').agg({
                'amount_logged': 'sum',
                'time_logged': 'sum'
            })
        ...
        avg_daily_time = total_time / total_days_in_period if total_days_in_period > 0 else 0
        
        logged_days = len(np.unique(period_df.index.date))
        days_with_logs_percentage = (logged_days / total_days_in_period * 100) if total_days_in_period > 0 else 0
        ...
        if most_logged_item is not None and most_logged_item['media_name']:
            ...
            item_entries = most_logged_item['entry_count']
            item_time = most_logged_item['time_logged']
            item_amount = most_logged_item['amount_logged']
            item_hours = int(item_time // 60)
            item_minutes = int(item_time % 60)
            item_time_str = f"{item_hours}h {item_minutes}m" if item_hours > 0 else f"{item_minutes}m"
        ...
        if len(category_stats) > 1:
            ...
            for _, row in category_stats.head(5).iterrows():
                ...
                breakdown_lines.append(f"• **{cat_name}**: {cat_time:.1f} min ({percentage:.1f}%) - {cat_amount:.0f} {unit_name}{'s' if cat_amount > 1 else ''}")
        ...
        most_logged_dict = dict(most_logged_item) if most_logged_item is not None else None
