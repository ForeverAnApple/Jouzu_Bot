...
from lib.immersion_helpers import (
    get_achievement_reached_info,
    get_current_and_next_achievement,
)
...
async def log_undo_autocomplete(interaction: discord.Interaction, current_input: str):
    ...
    for log_id, media_type, media_name, amount_logged, log_date in user_logs:
        ...
        if current_input.lower() in log_name.lower():
            choices.append(
                discord.app_commands.Choice(name=log_name, value=str(log_id))
            )
...
async def log_name_autocomplete(interaction: discord.Interaction, current_input: str):
    ...
    if MEDIA_TYPES[media_type]["autocomplete"]:
        result = await MEDIA_TYPES[media_type]["autocomplete"](
            interaction, current_input
        )
        return result
    return []
...
class ImmersionLog(commands.Cog):
    ...
    async def log(
        ...
        if not amount and not time_mins:
            return await interaction.response.send_message(
                "Please enter either an amount or a time to log. Or both.",
                ephemeral=True,
            )
        if amount and not amount.isdigit():
            return await interaction.response.send_message(
                "Amount must be a valid number.", ephemeral=True
            )
        elif amount:
            amount = int(amount)
            if amount < 0:
        ...
        if amount and amount > allowed_limit:
            return await interaction.response.send_message(
                f"Amount must be less than {allowed_limit} for `{MEDIA_TYPES[media_type]['log_name']}`.",
                ephemeral=True,
            )

        if name and len(name) > 150:
            return await interaction.response.send_message(
                "Name must be less than 150 characters.", ephemeral=True
            )
        elif name:
            name = name.strip()
        ...
        if time_mins and not time_mins.isdigit():
            return await interaction.response.send_message(
                "Time tracking must be a valid number.", ephemeral=True
            )
        elif time_mins:
            time_mins = int(time_mins)
            if time_mins < 1 or time_mins > 1440:
                return await interaction.response.send_message(
                    "Time tracking must be between 1 - 1440", ephemeral=True
                )

        if unit_is_time and time_mins and amount:
            return await interaction.response.send_message(
                "This tracking is pure time based, no need to enter time twice.",
                ephemeral=True,
            )

        if comment and len(comment) > 200:
            return await interaction.response.send_message(
                "Comment must be less than 200 characters.", ephemeral=True
            )
        elif comment:
        ...
        if mined and not mined.isdigit():
            return await interaction.response.send_message(
                "Mined must be a valid number.", ephemeral=True
            )
        elif mined:
            mined = int(mined)
            if mined < 0:
                return await interaction.response.send_message(
                    "Mined must be a positive number.", ephemeral=True
                )
        else:
            mined = 0
        ...
        if backfill_date is None:
            ...
            try:
                ...
                if log_date.date() > today:
                    return await interaction.response.send_message(
                        "You cannot log a date in the future.", ephemeral=True
                    )
                if (today - log_date.date()).days > 7:
                    return await interaction.response.send_message(
                        "You cannot log a date more than 7 days in the past.",
                        ephemeral=True,
                    )
        ...
        total_achievement_units_before = (
            await self.get_total_units_for_achievement_group(user.id, achievement_group)
        )
        ...
        (
            ...
            current_achievement,
            next_achievement,
        ) = await get_achievement_reached_info(
            achievement_group,
            total_achievement_units_before,
            total_achievement_units_after,
        )

        # Check achievement for total immersion time overall
        (
            immersion_achievement_reached,
            current_immersion_achievement,
            next_immersion_achievement,
        ) = await get_achievement_reached_info(
            "Immersion", current_total_time_before, current_total_time_after
        )
        ...
        log_embed.description = (
            f"[{actual_title}]({source_url})" if source_url else actual_title
        )
        ...
        log_embed.add_field(
            name="Total Minutes/Month",
            value=f"`{current_month_time_before}` → `{current_month_time_after}`",
        )
        log_embed.add_field(
            name="Total Mined/Month",
            value=f"`{current_month_mined_before}` → `{current_month_mined_after}`",
        )
        log_embed.add_field(
            name="Streak",
            value=f"{consecutive_days} day{'s' if consecutive_days > 1 else ''}",
        )
        log_embed.add_field(
            name=f"Category: {unit_name}(s) Received", value=amount_logged
        )
        log_embed.add_field(
            name=f"Total {unit_name}s",
        ...
        if achievement_reached and current_achievement:
            log_embed.add_field(
                name=f"{media_type} Achievement Reached! 🎉",
                value=current_achievement["title"],
                inline=False,
            )
        if next_achievement:
            ...
            log_embed.add_field(
                name=f"Next {media_type} Achievement",
                value=next_achievement_info,
                inline=False,
            )
        ...
        if immersion_achievement_reached and current_immersion_achievement:
            log_embed.add_field(
                name="Total Immersion Time Achievement Reached! 🎉",
                value=current_immersion_achievement["title"],
                inline=False,
            )
        if next_immersion_achievement:
            ...
            log_embed.add_field(
                name="Next Total Immersion Time Achievement",
                value=next_immersion_achievement_info,
                inline=False,
            )
        ...
        log_embed.set_footer(
            text=f"Logged by {user.display_name} for {log_date.split(' ')[0]}",
    ...
    async def get_thumbnail_url(self, media_type: str, name: str) -> Optional[str]:
        if MEDIA_TYPES[media_type]["thumbnail_query"]:
            result = await self.bot.GET(
                MEDIA_TYPES[media_type]["thumbnail_query"], (name,)
            )
            if result:
                return result[0][0]
        return None
    ...
    async def get_source_url(self, media_type: str, name: str) -> Optional[str]:
        if not MEDIA_TYPES[media_type]["source_url"]:
            return None
        exists_in_db = await self.bot.GET(
            MEDIA_TYPES[media_type]["title_query"], (name,)
        )
        if not exists_in_db:
            return None
        if media_type == "Listening Time":
            tmdb_media_type = await self.bot.GET(
                CACHED_TMDB_GET_MEDIA_TYPE_QUERY, (name,)
            )
            tmdb_media_type = tmdb_media_type[0][0]
            return (
                MEDIA_TYPES[media_type]["source_url"].format(
    ...
    @discord.app_commands.command(
        name="log_undo", description="Undo a previous immersion log!"
    )
    ...
    async def log_undo(self, interaction: discord.Interaction, log_entry: str):
        if not log_entry.isdigit():
            return await interaction.response.send_message(
                "Invalid log entry selected.", ephemeral=True
            )
        ...
        if log_id not in log_ids:
            return await interaction.response.send_message(
                "The selected log entry does not exist or does not belong to you.",
                ephemeral=True,
            )

        deleted_log_info = await self.bot.GET(
    ...
    async def get_total_units_for_achievement_group(
        self, user_id: int, achievement_group: str
    ) -> float:
        result = await self.bot.GET(
            GET_TOTAL_UNITS_FOR_ACHIEVEMENT_GROUP_QUERY, (user_id, achievement_group)
        )
    ...
    async def log_achievements(
        ...
        achievements_list = []
        achievements_dict = {
            settings_group["Achievement_Group"]: settings_group["unit_name"]
            for settings_group in MEDIA_TYPES.values()
        }
        achievements_dict["Immersion"] = "minute"

        for achievement_group, unit_name in achievements_dict.items():
            total_units = await self.get_total_units_for_achievement_group(
                user_id, achievement_group
            )
            ...
            (
                current_achievement,
                next_achievement,
            ) = await get_current_and_next_achievement(achievement_group, total_units)
    ...
    async def logs(
        ...
        if not user_logs:
            return await interaction.followup.send(
                f"No logs to export for {member.display_name}.", ephemeral=True
            )
        ...
        with open(log_filepath, mode="w", encoding="utf-8") as log_file:
            for log in user_logs:
                ...
                amount_logged = log[4]
                unit_name = (
                    MEDIA_TYPES[media_type]["unit_name"] + "s"
                    if amount_logged > 1
                    else MEDIA_TYPES[media_type]["unit_name"]
                )
        ...
        await interaction.followup.send(
            f"Here are {member.display_name}'s immersion logs:",
            file=discord.File(log_filepath),
        )
        os.remove(log_filepath)

    @discord.app_commands.command(
        name="log_server_report", description="Server wide immersion statistics."
    )
    @discord.app_commands.describe(
        media_type="Optionally specify the media type for leaderboard filtering.",
        month='Optionally specify the month in YYYY-MM format or select all with "ALL".',
    )
    @discord.app_commands.choices(media_type=LOG_CHOICES)
    @discord.app_commands.guild_only()
    async def log_server_report(
        self,
        interaction: discord.Interaction,
        ...
        if not month:
            ...
            try:
                ...
                return await interaction.followup.send(
                    "Invalid month format. Please use YYYY-MM.", ephemeral=True
                )

        leaderboard_data = await self.bot.GET(
            GET_MONTHLY_LEADERBOARD_QUERY,
            (month, month, media_type, media_type, guild_id),
        )
        ...
        if leaderboard_data:
            for rank, (user_id, total_time, total_units) in enumerate(
