...
async def check_immersion_goal_status(bot: JouzuBot, user_id: int):
    ...
    for goal_id, goal_type, goal_value, end_date, created_at, progress in result:
        ...
        created_at_dt = datetime.strptime(created_at, "%Y-%m-%d %H:%M:%S").replace(
            tzinfo=timezone.utc
        )
...
class GoalsCog(commands.Cog):
    ...
    async def log_set_goal(
        ...
        try:
            if end_date_or_hours.isdigit():
                ...
                end_date_dt = datetime.strptime(end_date_or_hours, "%Y-%m-%d").replace(
                    tzinfo=timezone.utc
                )
    ...
    async def log_clear_goals(self, interaction: discord.Interaction):
        ...
        if not expired_goals:
            return await interaction.response.send_message(
                f"> You have no expired goals to clear.", ephemeral=True
            )
